...
function handleLoggerGroup(logger: Logger, host: ts.server.ServerHost | undefined): Logger {
    let inGroup = false;
    ...
    logger.info = s => msg(s, ts.server.Msg.Info);
    logger.log = write;
    return logger;

    function msg(s: string, type = ts.server.Msg.Err) {
...
export function nowString(host: ts.server.ServerHost) {
    // E.g. "12:34:56.789"
    host.now?.(); // To increment the time but not print it to avoid the baseline updates
    return `hh:mm:ss:mss`;
}

export function createLoggerWritingToConsole(host: ts.server.ServerHost): Logger {
...
export function sanitizeLog(s: string): string {
    ...
    s = s.replace(/getIndentationAtPosition: getCurrentSourceFile: \d+(?:\.\d+)?/g, `getIndentationAtPosition: getCurrentSourceFile: *`);
    s = s.replace(/getIndentationAtPosition: computeIndentation\s+: \d+(?:\.\d+)?/g, `getIndentationAtPosition: computeIndentation  : *`);
