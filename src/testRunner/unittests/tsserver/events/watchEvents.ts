...
describe("unittests:: tsserver:: events:: watchEvents", () => {
    ...
    function addFile(session: TestSession, path: string) {
        ...
        (session.logger.host as TestServerHostWithCustomWatch).factoryData.watchedDirectoriesRecursive.get("/user/username/projects/myproject")?.forEach(data => session.executeCommandSeq<ts.server.protocol.WatchChangeRequest>({
    ...
    function changeFile(session: TestSession, path: string) {
        ...
        session.logger.log("Custom watch");
        (session.logger.host as TestServerHostWithCustomWatch).factoryData.watchedFiles.get(path)?.forEach(data => session.executeCommandSeq<ts.server.protocol.WatchChangeRequest>({
            command: ts.server.protocol.CommandTypes.WatchChange,
